    ...
    useEffect(() => {
        useEffect(() => {
            ...
            socket.on('alarm_triggered', (alarm) => {
                ...
                if (dismiss) {
                    if (dismiss) {
                        fetch(`${API_BASE}/clock/alarms/${alarm.id}/dismiss`, { method: 'POST' });
                    }
                }
            });
        ...
        const handleAddAlarm = async () => {
            ...
            try {
                try {
                    const res = await fetch(`${API_BASE}/clock/alarms`, {
                        method: 'POST',
            ...
            const handleDeleteAlarm = async (id) => {
                try {
                    const handleDeleteAlarm = async (id) => {
                        try {
                            await fetch(`${API_BASE}/clock/alarms/${id}`, { method: 'DELETE' });
                    ...
                    const handleToggleAlarm = async (id, currentStatus) => {
                        try {
                            const handleToggleAlarm = async (id, currentStatus) => {
                                try {
                                    await fetch(`${API_BASE}/clock/alarms/${id}/toggle`, {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
